...
public:
	...
	template<typename T>
	typename std::enable_if<(std::is_enum<T>::value && valid_type<typename std::underlying_type<T>::type>::value), T>::type
		read()
	{
		return static_cast<T>(read<typename std::underlying_type<T>::type>());
	}
	template<typename T>
	typename std::enable_if<(!std::is_enum<T>::value && valid_type<T>::value), T>::type read()
	{
